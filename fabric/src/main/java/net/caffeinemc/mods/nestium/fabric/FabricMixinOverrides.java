...
public class FabricMixinOverrides implements PlatformMixinOverrides {
    ...
    public List<MixinOverride> applyModOverrides() {
        ...
        for (ModContainer container : FabricLoader.getInstance().getAllMods()) {
            ...
            if (meta.containsCustomValue(JSON_KEY_NESTIUM_OPTIONS)) {
                CustomValue overrides = meta.getCustomValue(JSON_KEY_NESTIUM_OPTIONS);

                if (overrides.getType() != CustomValue.CvType.OBJECT) {
                    System.out.printf("[Nestium] Mod '%s' contains invalid Nestium option overrides, ignoring", meta.getId());
                    continue;
                }

                for (Map.Entry<String, CustomValue> entry : overrides.getAsObject()) {
                    if (entry.getValue().getType() != CustomValue.CvType.BOOLEAN) {
                        System.out.printf("[Nestium] Mod '%s' attempted to override option '%s' with an invalid value, ignoring", meta.getId(), entry.getKey());
                        continue;
                    }
