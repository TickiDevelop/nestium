...
package net.caffeinemc.mods.nestium.client.render.frapi.mesh;
...
import net.caffeinemc.mods.nestium.client.render.frapi.material.RenderMaterialImpl;
...
public abstract class EncodingFormat {
    ...
    static {
        ...
        Preconditions.checkState(VERTEX_STRIDE == QuadView.VANILLA_VERTEX_STRIDE, "Nestium FRAPI vertex stride (%s) mismatched with rendering API (%s)", VERTEX_STRIDE, QuadView.VANILLA_VERTEX_STRIDE);
        Preconditions.checkState(QUAD_STRIDE == QuadView.VANILLA_QUAD_STRIDE, "Nestium FRAPI quad stride (%s) mismatched with rendering API (%s)", QUAD_STRIDE, QuadView.VANILLA_QUAD_STRIDE);
    }
    ...
    static {
        Preconditions.checkArgument(MATERIAL_SHIFT + MATERIAL_BIT_COUNT <= 32, "Nestium FRAPI header encoding bit count (%s) exceeds integer bit length)", TOTAL_STRIDE);
    }
