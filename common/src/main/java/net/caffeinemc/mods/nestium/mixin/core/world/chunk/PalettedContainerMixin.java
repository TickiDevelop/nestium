package net.caffeinemc.mods.nestium.mixin.core.world.chunk;

import net.caffeinemc.mods.nestium.client.world.BitStorageExtension;
...
public abstract class PalettedContainerMixin<T> implements PalettedContainerROExtension<T> {
    ...
    @Override
    public void nestium$unpack(T[] values) {
        ...
        storage.nestium$unpack(values, data.palette());
    }

    @Override
    public void nestium$unpack(T[] values, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        var strategy = Objects.requireNonNull(this.strategy);
    ...
    @Override
    public PalettedContainerRO<T> nestium$copy() {
        return this.copy();
    }
}
