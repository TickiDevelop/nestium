...
public interface PalettedContainerROExtension<T> {
    ...
    static <T> PalettedContainerRO<T> clone(PalettedContainerRO<T> container) {
        ...
        return of(container).nestium$copy();
    }

    void nestium$unpack(T[] values);
    void nestium$unpack(T[] values, int minX, int minY, int minZ, int maxX, int maxY, int maxZ);

    PalettedContainerRO<T> nestium$copy();
}
