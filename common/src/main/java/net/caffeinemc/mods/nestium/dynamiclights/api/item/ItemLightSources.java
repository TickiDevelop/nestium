...
public final class ItemLightSources {
	...
	private static void load(ResourceLocation resourceId, Resource resource) {
		...
		try (var reader = new InputStreamReader(resource.open())) {
			...
			NestiumDynamicLights.get().warn("Failed to load item light source \"" + id + "\".");
		}
	}
	...
	private static void register(ItemLightSource data) {
		...
		if (other != null) {
			NestiumDynamicLights.get().warn("Failed to register item light source \"" + data.id() + "\", duplicates item \""
