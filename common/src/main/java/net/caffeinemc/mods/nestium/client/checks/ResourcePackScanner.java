...
public class ResourcePackScanner {
    private static final Logger LOGGER = LoggerFactory.getLogger("Nestium-ResourcePackScanner");
    ...
    /**
     * <a href="https://github.com/CaffeineMC/nestium/issues/1569">#1569</a>
    ...
    private static void printToasts(Collection<ScannedResourcePack> resourcePacks) {
        ...
        if (!incompatibleResourcePacks.isEmpty()) {
            showConsoleMessage("nestium.console.core_shaders_error", true, MessageLevel.SEVERE);
        ...
        if (!likelyIncompatibleResourcePacks.isEmpty()) {
            showConsoleMessage("nestium.console.core_shaders_warn", true, MessageLevel.WARN);
        ...
        if (shown) {
            showConsoleMessage("nestium.console.core_shaders_info", true, MessageLevel.INFO);
        }
    }
        ...
        for (var entry : scanResults) {
            ...
            if (!entry.shaderPrograms.isEmpty()) {
                emitProblem(builder,
                        ...
                        "https://github.com/CaffeineMC/nestium/wiki/Resource-Packs",
     ...
     * which will not work with Nestium, but that Nestium can ignore.
     *
    ...
    private static List<String> determineIgnoredShaders(PackResources resourcePack) {
        ...
        try {
            var meta = resourcePack.getMetadataSection(NestiumResourcePackMetadata.SERIALIZER);
            if (meta != null) {
