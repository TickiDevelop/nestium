	...
	static <T extends LivingEntity> @NotNull DynamicLightHandler<T> makeLivingEntityHandler(@NotNull DynamicLightHandler<T> handler) {
		return entity -> {
			...
			for (var equipmentSlot : EquipmentSlot.values()) {
				...
				luminance = Math.max(luminance, NestiumDynamicLights.getLuminanceFromItemStack(equipped, entity.isUnderWater()));
			}
		...
		return new DynamicLightHandler<>() {
			...
			public int getLuminance(T entity) {
				...
				if (entity.getSwelling(0.f) > 0.001) {
					luminance = switch (net.caffeinemc.mods.nestium.client.NestiumClientMod.options().dynamicLights.creeper) {
						case OFF -> 0;
						case SIMPLE -> 10;
