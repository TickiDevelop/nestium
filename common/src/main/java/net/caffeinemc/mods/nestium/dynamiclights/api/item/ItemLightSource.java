...
public abstract class ItemLightSource {
	...
	public int getLuminance(ItemStack stack, boolean submergedInWater) {
		if (this.waterSensitive() && net.caffeinemc.mods.nestium.client.NestiumClientMod.options().dynamicLights.waterSensitiveCheck && submergedInWater)
	...
	public static @NotNull Optional<ItemLightSource> fromJson(@NotNull ResourceLocation id, @NotNull JsonObject json) {
		if (!json.has("item") || !json.has("luminance")) {
			NestiumDynamicLights.get().warn("Failed to parse item light source \"" + id + "\", invalid format: missing required fields.");
			return Optional.empty();
		}

		var affectId = ResourceLocation.tryParse(json.get("item").getAsString());

		var ref = BuiltInRegistries.ITEM.getOptional(affectId);
		if (ref.isEmpty())
			return Optional.empty();
		...
		if (luminanceElement.isNumber()) {
			...
			if (luminanceStr.equals("block")) {
				...
				if (blockId != null) {
					var blockRef = BuiltInRegistries.BLOCK.getOptional(blockId);
			...
			NestiumDynamicLights.get().warn("Failed to parse item light source \"" + id + "\", invalid format: \"luminance\" field value isn't string or integer.");
		}
