...
import net.caffeinemc.mods.nestium.gnetum.CacheSetting;
import net.caffeinemc.mods.nestium.gnetum.Gnetum;
import net.caffeinemc.mods.nestium.gnetum.PerformanceAnalyzer;
import net.caffeinemc.mods.nestium.gnetum.gui.widgets.IconButton;
...
public class ConfigScreen extends BaseScreen {
    ...
    protected void rebuild() {
        super.rebuild();
        var btnEnabled = new ToggleButton(width / 2 - 130, height / 2 - 90, 120, 20, Gnetum.config.enabled, () -> I18n.get("nestium.gnetum.config.enabled") + ": %s");
        btnEnabled.setTooltip(Tooltip.create(Component.translatable("nestium.gnetum.config.enabled.tooltip")));
        btnEnabled.setTooltipDelay(Duration.ZERO);
        int w1 = analysis == null ? 120 : 120 - 20;
        btnMoreOptions = Button
                .builder(Component.translatable("nestium.gnetum.config.moreOptions"), b -> { Minecraft.getInstance().setScreen(new MoreOptionsScreen()); })
        ...
        if (analysis != null) {
            ...
            Component tooltip = Component.translatable(
                    analysis.isOutdated() ? "nestium.gnetum.config.analysis.outdated" :
                            (analysis.getMessages().isEmpty() ? "nestium.gnetum.config.analysis.good" : "nestium.gnetum.config.analysis.suboptimal")
            );
        ...
        btnModdedPre = Button
                .builder(Component.translatable("nestium.gnetum.config.moddedPre"),
        ...
        y += margin + h;
        btnVanilla = Button
                .builder(Component.translatable("nestium.gnetum.config.vanilla"),
        ...
        btnModdedPost = Button
                .builder(Component.translatable("nestium.gnetum.config.moddedPost"),
